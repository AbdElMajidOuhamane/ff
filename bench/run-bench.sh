...
#   Appendix: compute-bound workloads (QuickJS = no JIT) — labeled
...
run_group() {
    ...
    for name in "$@"; do
        ...
        hyperfine -N --warmup 5 --runs 100 --style basic \
            ...
            --command-name Deno "deno run -A $file" \
...
run_group "TIER 2 — General JS workloads" closure string fetch timer
run_group "APPENDIX — Compute-bound (QuickJS = no JIT; not representative of I/O workloads)" object sort fib json
