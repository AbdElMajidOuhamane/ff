...
pub const TimerManager = struct {
    loop: *xev.Loop,
    // One kernel-timer handle per slot instead of one shared handle armed by
    // every slot: arming slot B can override slot A's pending deadline on a
    // shared handle, so overlapping setTimeout/setInterval intervals get
    // their own identity.
...
fn timerCallback(
    ...
    // Canceled: cancelCb owns reclamation (freeing here too would push the
    // same slot onto the free list twice). Other errors previously leaked
    // the slot forever — reclaim it.
    _ = r catch |err| {
