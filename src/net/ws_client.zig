...
// ============================================================
// Client WebSocket — DOD/SoA slot pool, same idiom as async_fetch.
//
// Every live client is an integer slot id into dense per-field arrays
// (one contiguous block per field, TigerBeetle style). The pool doubles
// as the concurrency cap (at most MAX_WS workers exist). Worker completion
// is detected with a SIMD sweep over the state vector (doneMask), then
// confirmed per-slot with an acquire CAS — vector wide, scalar per-element.
//
// Cross-thread wakeup: one xev.Async (mach port) + per-slot nonblocking
// pipe wakeups. The worker poll()s [sock_fd, pipe_r]; the main thread
// pokes pipe_w to deliver a send/close command within one poll timeout
// (<= 50ms).
//
// v8 is touched ONLY on the main thread (drainCompleted/dispatch).
//
// Event ordering is a strict handshake: the worker signals JOB_DONE, then
// spins until the main thread ack'd it back to JOB_BUSY before emitting the
// next event — so OPEN -> MESSAGE* -> CLOSE always drain in order.
//
// TX hand-off is LOCKLESS (skill rule: keep locks out of the data path):
// main fills tx_msg only while tx_pending is false, publishes with a
// release-store; the worker acquires the same flag, copies out, and
// releases the slot back. Single-writer alternation — the buffer is only
// ever touched by whichever side owns the flag phase, so no lock is needed.
// The spinlock-guarded free-list remains exclusively for slot alloc/free.
//
// `pending` counts LIVE WORKER THREADS: the decrement runs as the FIRST
// defer in workerMain so it executes only after every cleanup defer has
// finished — the loop cannot exit while a worker is still winding down.
// ============================================================
pub const MAX_WS = 64; // == u64 mask lanes (doneMask)
pub const WS_MSG_SIZE = ws.WS_MSG_SIZE;
pub const RDBUF = 65536; // >= std client tls/read buffer, so one read drains all
const CLOSE_REASON_MAX = 123; // RFC6455: close payload <= 125, minus 2-byte code
const JOB_FREE: u8 = 0;
const JOB_BUSY: u8 = 1;
const JOB_DONE: u8 = 2;
...
var keys:         [MAX_WS][28]u8 = undefined; // base64 sec-websocket-key (24 used)
...
// ---- tx (main -> worker, lockless publish-flag mailbox) ----
...
// ---- rx (worker -> main; ordering via state CAS) ----
...
// ---- worker-side scratch / read state ----
...
// ---- cold bulk: out-of-line so hot state stays dense ----
var rd_buf:       [MAX_WS][RDBUF]u8 = undefined;
var partial:      [MAX_WS][WS_MSG_SIZE]u8 = undefined;
// ---- O(1) free-list slot allocator (spinlock-guarded, ALLOC/FREE ONLY) ----
...
// main-thread/pump-visible live-worker counter; 0 => all workers exited
pub var pending: std.atomic.Value(u64) = .{ .raw = 0 };
// ---- V8 string globals (rooted once in setupStrings) ----
var str_onopen: c.Global = .{ .data_ptr = 0 };
var str_onmessage: c.Global = .{ .data_ptr = 0 };
var str_onclose: c.Global = .{ .data_ptr = 0 };
var str_onerror: c.Global = .{ .data_ptr = 0 };
var str_ready_state: c.Global = .{ .data_ptr = 0 };
var str_data: c.Global = .{ .data_ptr = 0 };
var str_code: c.Global = .{ .data_ptr = 0 };
var str_reason: c.Global = .{ .data_ptr = 0 };
var str_message: c.Global = .{ .data_ptr = 0 };
...
fn releaseSlot(s: usize) void {
    if (socks[s].data_ptr != 0) c.v8__Global__Reset(&socks[s]);
    socks[s] = .{ .data_ptr = 0 };
    gpa.free(hosts[s]);
    gpa.free(paths[s]);
    hosts[s] = "";
...
pub fn setupStrings(isolate: ?*c.Isolate) void {
    ...
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "onopen", 0, -1)), &str_onopen);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "onmessage", 0, -1)), &str_onmessage);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "onclose", 0, -1)), &str_onclose);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "onerror", 0, -1)), &str_onerror);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "readyState", 0, -1)), &str_ready_state);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "data", 0, -1)), &str_data);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "code", 0, -1)), &str_code);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "reason", 0, -1)), &str_reason);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "message", 0, -1)), &str_message);
    c.v8__Global__New(isolate, @ptrCast(c.v8__String__NewFromUtf8(isolate, "type", 0, -1)), &str_type);
}
// ---- submission (main thread, from the WebSocket constructor) ----
// Takes ownership of host/path (dupes). Returns the slot id.
pub fn submit(
    isolate: ?*c.Isolate,
    obj: *const c.Value,
    host: []const u8,
...
fn makePipe(s: usize) !void {
    // Zig 0.16 removed pipe2 from std.posix. Create via the libc shim, then
    // flip O.NONBLOCK at the type level (O is a packed-struct flag set on
    // darwin and linux) so serviceTx's drain loop terminates on EAGAIN
    // instead of parking forever on an empty blocking pipe.
...
// ---- send / close (main thread, from JS methods) ----
pub fn sendBytes(s: usize, bytes: []const u8, binary: bool) void {
    ...
    // Lockless mailbox: fill only while the slot is empty, then publish.
    if (tx_pending[s].load(.acquire)) return; // drop-new (mirrors server staging)
...
pub fn closeWs(s: usize, code: u16, reason: []const u8) void {
    if (states[s].load(.acquire) == JOB_FREE) return;
    // close always wins over a queued data frame
...
fn workerMain(slot_id: u16) void {
    ...
    // Declared FIRST so it executes LAST: pending drops only after all
    // cleanup defers below have completed. The event loop keys off pending,
    // so teardown can't race a live worker thread.
    ...
    while (true) {
        ...
        // For TLS, drain immediately whenever a full ciphertext record is
        // already buffered or there is decrypted plaintext or socket data
        // waiting; otherwise park up to 50ms (also bounds TX latency).
        ...
        if (tls_ready or (rc != 0 and pfd[0].revents != 0)) {
            ...
            if (n == 0) {
                // TLS: 0 == "not ready yet" (partial record / no new data);
                // keep polling. Plain: 0 == real EOF.
...
fn cleanupConn(s: usize, conn: ?*http.Client.Connection, r: *?http.Client.Request) void {
    ...
    sock_fds[s] = -1;
    if (tls_active[s]) {
        // Hand the socket to us and keep req.deinit() from re-releasing it or
        // draining the live WS connection (Client.zig discard path).
...
// Signals the main thread that results are ready. Does NOT touch `pending`:
// the liveness counter is decremented by workerMain's first defer instead,
// after all cleanup defers have run.
...
// Strict event handshake: the worker won't emit a second event until the
// main thread has consumed the previous one (state returned to JOB_BUSY).
...
// ---- V8 ArrayBuffer backing-store access (same shim symbol as crypto.zig) ----
extern fn std__shared_ptr__v8__BackingStore__get(self: *const c.SharedPtr) callconv(.c) ?*c.BackingStore;

// ============================================================
// Plain-TCP upgrade handshake (wss:// is handled by std.http above)
// ============================================================
...
/// TLS read readiness: plaintext already sitting in the connection's
/// buffered reader, or ciphertext waiting on the raw socket.
fn tlsReadReady(s: usize, cn: *http.Client.Connection) bool {
    if (cn.reader().bufferedLen() > 0) return true;
    return sockReadable(s, 0);
}

/// Plain connections: read straight off the fd (the connection's buffered
/// reader stalls when pulled from this worker thread — same issue the
/// handshake hit; the fd path is proven by the 101 exchange).
/// TLS connections: MUST go through the reader (decryption lives there).
...
fn verifyUpgrade(s: usize, cn: *http.Client.Connection) bool {
    _ = cn; // steady-state reads use the reader; the handshake reads the fd
    ...
    while (head_end == null) {
        ...
        // Buffered plaintext counts as readable even if the kernel socket
        // has nothing pending.
        if (sock_fds[s] < 0) return false;
        var pfd = [_]std.posix.pollfd{
            .{ .fd = sock_fds[s], .events = std.posix.POLL.IN, .revents = 0 },
        ...
        if (n == 0) return false; // EOF mid-handshake
    ...
    // Frame bytes that rode along on the handshake segment move into rd_buf
    // so consumeFrames picks them up right after open (nothing is lost).
...
// ============================================================
// Frame send/receive (worker thread)
// ============================================================
/// Client->server frames MUST be masked (RFC6455 5.1).
...
/// We initiate the close (or an error close): record rx side so closeExit
/// keeps our code instead of defaulting to 1006, then send the frame.
...
/// Drain every complete frame currently in rd_buf. Leftover partial bytes
/// are compacted to the front for the next read. False = stop the session.
...
/// Wakeup bytes drained until EAGAIN (nonblocking pipe), then the tx
/// mailbox is consumed: acquire-load the publish flag, copy the message
/// OUT of shared storage, release the slot — framing/writing happens on
/// private stacks outside any shared state.
fn serviceTx(s: usize, cn: *http.Client.Connection) bool {
    var wake: [64]u8 = undefined;
    while (true) {
        const n = std.c.read(pipe_fds[s][0], &wake, wake.len);
        if (n <= 0) break; // EAGAIN (drained) or EOF
    }
...
// ============================================================
// Main-thread pump: arm / sweep / claim / dispatch (V8 here only)
// ============================================================
...
fn doneMask() u64 {
    ...
    for (0..MAX_WS) |i| raw[i] = states[i].raw; // hint-only gather
...
/// Dispatch finished WS events. Call each loop tick (after loop.run,
/// before pumping microtasks) so the worker handshake advances promptly.
pub fn drainCompleted(isolate: ?*c.Isolate) void {
    var mask = doneMask();
    while (mask != 0) {
        const s: usize = @ctz(mask);
        mask &= mask - 1;
        if (claimSlot(s)) completeEvent(isolate, s);
    }
}

pub fn setReadyState(obj: *const c.Value, context: ?*c.Context, isolate: ?*c.Isolate, v: i32) void {
    var out: c.MaybeBool = undefined;
    _ = c.v8__Object__Set(
        @ptrCast(obj),
        context,
        @ptrCast(c.v8__Global__Get(&str_ready_state, isolate)),
        @ptrCast(c.v8__Integer__New(isolate, v)),
        &out,
    );
}
...
fn dispatchEvent(
    isolate: ?*c.Isolate,
    context: ?*c.Context,
    sock_val: *const c.Value,
    name_g: *c.Global,
    event_val: ?*const c.Value,
) void {
    const name = c.v8__Global__Get(name_g, isolate) orelse return;
    const prop = c.v8__Object__Get(@ptrCast(sock_val), context, name);
    if (prop == null or !c.v8__Value__IsFunction(prop)) return;
    var argv: [1]?*const c.Value = .{event_val};
    const argc: c_int = if (event_val == null) 0 else 1;
    _ = c.v8__Function__Call(@ptrCast(prop), context, sock_val, argc, &argv);
}

fn newEventObj(isolate: ?*c.Isolate, context: ?*c.Context, comptime type_name: []const u8) *const c.Value {
    const obj = c.v8__Object__New(isolate);
    var out: c.MaybeBool = undefined;
    _ = c.v8__Object__Set(
        @ptrCast(obj),
        context,
        @ptrCast(c.v8__Global__Get(&str_type, isolate)),
        c.v8__String__NewFromUtf8(isolate, @ptrCast(type_name.ptr), 0, @intCast(type_name.len)),
        &out,
    );
    return @ptrCast(obj);
}

fn makeOpenEvent(isolate: ?*c.Isolate, context: ?*c.Context) *const c.Value {
    return newEventObj(isolate, context, "open");
}

fn makeMessageEvent(isolate: ?*c.Isolate, context: ?*c.Context, s: usize) *const c.Value {
    const obj = newEventObj(isolate, context, "message");
    var out: c.MaybeBool = undefined;
    const n = @min(rx_len[s], WS_MSG_SIZE);
    const data_val: ?*const c.Value = if (rx_binary[s]) b: {
        const ab = c.v8__ArrayBuffer__New(isolate, n);
        const store = c.v8__ArrayBuffer__GetBackingStore(ab);
        if (std__shared_ptr__v8__BackingStore__get(&store)) |bs| {
            if (c.v8__BackingStore__Data(bs)) |p| {
                const dst: [*]u8 = @ptrCast(p);
                @memcpy(dst[0..n], rx_msg[s][0..n]);
            }
        }
        // Wrap in a Uint8Array view (browser parity): JS handlers check
        // `ev.data instanceof Uint8Array`.
        break :b @ptrCast(c.v8__Uint8Array__New(@ptrCast(ab), 0, n));
    } else c.v8__String__NewFromUtf8(isolate, @ptrCast(&rx_msg[s]), 0, @intCast(n));
    if (data_val) |dv| {
        _ = c.v8__Object__Set(
            @ptrCast(obj),
            context,
            @ptrCast(c.v8__Global__Get(&str_data, isolate)),
            dv,
            &out,
        );
    }
...
fn makeCloseEvent(isolate: ?*c.Isolate, context: ?*c.Context, s: usize) *const c.Value {
    const obj = newEventObj(isolate, context, "close");
    var out: c.MaybeBool = undefined;
    _ = c.v8__Object__Set(
        @ptrCast(obj),
        context,
        @ptrCast(c.v8__Global__Get(&str_code, isolate)),
        @ptrCast(c.v8__Integer__New(isolate, rx_code[s])),
        &out,
    );
    _ = c.v8__Object__Set(
        @ptrCast(obj),
        context,
        @ptrCast(c.v8__Global__Get(&str_reason, isolate)),
        c.v8__String__NewFromUtf8(isolate, @ptrCast(&rx_reason[s]), 0, @intCast(rx_reason_len[s])),
        &out,
    );
    return obj;
}

fn makeErrorEvent(isolate: ?*c.Isolate, context: ?*c.Context, s: usize) *const c.Value {
    const obj = newEventObj(isolate, context, "error");
    var out: c.MaybeBool = undefined;
    _ = c.v8__Object__Set(
        @ptrCast(obj),
        context,
        @ptrCast(c.v8__Global__Get(&str_message, isolate)),
        c.v8__String__NewFromUtf8(isolate, @ptrCast(&rx_err[s]), 0, @intCast(rx_err_len[s])),
        &out,
    );
    return obj;
}

fn completeEvent(isolate: ?*c.Isolate, s: usize) void {
    var hs: c.HandleScope = undefined;
    c.v8__HandleScope__CONSTRUCT(&hs, isolate);
    defer c.v8__HandleScope__DESTRUCT(&hs);
    ...
    if (ev == EV_CLOSE) {
        if (c.v8__Global__Get(&socks[s], isolate)) |sv| {
            if (rx_err_len[s] > 0) {
                dispatchEvent(isolate, context, sv, &str_onerror, makeErrorEvent(isolate, context, s));
            }
            setReadyState(sv, context, isolate, 3); // CLOSED
            dispatchEvent(isolate, context, sv, &str_onclose, makeCloseEvent(isolate, context, s));
        }
        releaseSlot(s); // terminal event: frees host/path/global, slot -> FREE
        return;
    }

    const sv = c.v8__Global__Get(&socks[s], isolate) orelse {
        states[s].store(JOB_BUSY, .release); // ack anyway so the worker unstalls
        return;
    };
    switch (ev) {
        EV_OPEN => {
            ws_open[s] = true; // publish readiness BEFORE dispatch so sends inside onopen work
            setReadyState(sv, context, isolate, 1); // OPEN
            dispatchEvent(isolate, context, sv, &str_onopen, makeOpenEvent(isolate, context));
            states[s].store(JOB_BUSY, .release);
        },
        EV_MESSAGE => {
            dispatchEvent(isolate, context, sv, &str_onmessage, makeMessageEvent(isolate, context, s));
            states[s].store(JOB_BUSY, .release);
        },
...
var socks: [MAX_WS]c.Global = [_]c.Global{.{ .data_ptr = 0 }} ** MAX_WS;
