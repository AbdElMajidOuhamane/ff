...
fn runJob(slot_id: u16) void {
    ...
    hop: while (true) {
        ...
        // FIXED: transfer heap headers ownership
        ...
        var header_it=response.head.iterateHeaders();
        while (header_it.next()) |h| { resp_data.headers.appendEntry(h.name,h.value); }
        if (has_body) {
            ...
            const reader = if (!compressed) response.reader(&transfer_buf) else dec:{
                ...
                decompress_buf=d; break :dec response.readerDecompressing(&transfer_buf, &decompress, d);
            };
            ...
            if (wire_cl) |cl| {
                if (cl>0) {
                    ...
                    if (buf) |b| {
                        ...
                        else if (n==len){ owned_body=b; }
                        else{ owned_body=gpa.realloc(b,n) catch null; if (owned_body==null) gpa.free(b); body_incomplete=true; }
                    }
                }
            } else {
                var acc: std.ArrayList(u8)=.empty; defer acc.deinit(gpa);
                ...
                while (true) {
                    ...
                    if (n==0) break;
                    acc.appendSlice(gpa,chunk[0..n]) catch |err|{ std.debug.print("[fetch] body accumulate error: {s}\n", .{@errorName(err)}); break; };
                    total+=n;
                }
                if (total>0){ owned_body=acc.toOwnedSlice(gpa) catch |err| fin_b:{ std.debug.print("[fetch] body finalize error: {s}\n", .{@errorName(err)}); break :fin_b null; }; }
            }
            if (compressed){
        ...
        if (owned_body) |b| { resp_data.setBodyOwned(b); owned_body=null; }
